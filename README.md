...
- **PREREQUISITE 2 🥁 —** The possibility to address "verticalities" (musical offsets expressed in rhythmic values), for example to annotate the score with root notes that have been identified by the analysts. These verticalities are not materialized in the MEI encoding.
...
Moreover, we need to extract all the offsets defined at the score level, so as to provide anchors for annotation that materialize root notes. Here is an overview of the RDF data that represent such a score offset:
